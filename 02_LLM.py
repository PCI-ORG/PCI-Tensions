...
prompt = '''You are an intelligence agent helping policymakers analye the international and political relations between China and Taiwan. You are tasked to analyze a Chinese newspaper article regarding Taiwan. Answer the following questions based on the title and body of a newspaper article:
{questions} 
...
questions_binary["Mention_Taiwan_negative_action"] = '''Does the article mention Taiwan's policy or actions will damage and hurt Chinese or Taiwanese welfare or lead to a negative consequecne? If so, give 1. If not, give 0.'''
...
questions_scale["Collaboration"] = '''Is there any collaboration or cooperation between China and Taiwan at the current moment, including but not limited to economic or trade agreements, cross-strait exchanges, official communications, culture and socail exchanges? Give a real number between 0 and 1.00 to indicate the current collaboration or cooperation between China and Taiwan. 1.00 indicates the highest collaboration, and 0 indicates the lowest collaboration. Give an O if the article does not mention anything related to collaboration or cooperation between China and Taiwan.'''
...
for questions_type in [questions_binary, questions_scale, questions_qualitative]:
    for topic, content in questions_type.items():
        ...
        question_propmt = question_propmt + f"{n}. {topic}: {content}\n"
        n += 1        
print(question_propmt)
...
class PCI_Taiwan_output_class(BaseModel):
...
title = dfs['96_crisis'].iloc[i].title
body = dfs['96_crisis'].iloc[i].body

structured_llm = model.with_structured_output(PCI_Taiwan_output_class)
output = structured_llm.invoke(prompt.format(questions=question_propmt, title=title, body=body))
...
try:
    df_results = pd.read_csv(f"Results/{event}_{date}.csv")
...
for _, i in enumerate(df_sample['id']):
    ...
    if _ % 10 == 1: 
        ...
        df_results.to_csv(f"Results/{event}_{date}.csv", index=False)
...
df_results.to_csv("Results/{event}_{date}.csv", index=False)
